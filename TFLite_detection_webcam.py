...
#
# Author: Evan Juras (Modified slightly by Joel Stremmel to save captured images)
# Date: 10/27/19
# Description:
...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            # Modification by Joel Stremmel
            # Write most recent image to image path
            # Sleep so that we don't just keep sending the same image
            print('Sending image...')
    ...
    # Commented for remote execution
